  ...
  async function readMessages(): Promise<void> {
    try {
      const result = await createConsumer(
        ...
        (error) => defaultLogger.log(`Skipping ${error}`),
      );
      ...
      const checkInterval = setInterval(() => {
        if (Date.now() - lastMessageTime > 2000) {
          ...
          if (messageCount > 0) {
            saveMapState(mapState, defaultLogger);
          }
      ...
      defaultLogger.error(`Error reading messages: ${error}`);
      process.exit(1);
    }
  }

  async function shutdown(): Promise<void> {
    defaultLogger.log('Shutting down...');
    if (consumer) {
